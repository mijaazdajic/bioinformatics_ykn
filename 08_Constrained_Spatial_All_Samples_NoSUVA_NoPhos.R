...
env.data <-  env.data %>% column_to_rownames(., var = "#SampleID") %>% 
  ...
  dplyr::rename(space.vector= 'space.ord$rotation[, 1]')
...
scaled.env <-  env.data %>% 
  ...
  dplyr::select("space.trans", "SUVA","DOC", "pH", "lnsul", "lnFe", "sqrtP")
...
anova(mod, by = "terms", permutations = 999) #space is not so significant, might slighly change since permuations
